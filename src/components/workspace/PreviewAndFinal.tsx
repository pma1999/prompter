  ...
  return (
    <Card className="mb-4">
      ...
      <CardContent className="space-y-3">
        <Textarea value={value} onChange={(e) => onChange(e.target.value)} className="text-sm bg-muted p-3 rounded-md resize-y min-h-[100px] max-h-[50vh] md:max-h-[320px]" aria-label="Preview prompt editor" />
        <div className="text-xs text-muted-foreground">Tip: This preview is editable. Copy and Insert will use your changes.</div>
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="text-xs text-muted-foreground">Tip: This prompt is editable. Copy actions will use your changes.</div>
        <div className="text-xs text-muted-foreground">
          {typeof usage?.cachedContentTokenCount === "number" && (
            <span>Cached {usage.cachedContentTokenCount} • </span>
          )}
